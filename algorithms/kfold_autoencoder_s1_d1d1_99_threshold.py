...
def find_threshold(model, scaled_train_data_x, value):
...
for (train, test), i in zip(cv.split(X, y), range(4)):
    ...
    list_of_thresholds = [99]
    for value in list_of_thresholds:
        threshold = find_threshold(model, scaled_train_data_x, value)
        print(f"Value: {value}")
        ...
        list_of_acuracy.append(accuracy)
    ...
    print(k)
    print(value)
    print("Accuracy:", list_of_acuracy)
    print("Precision:",list_of_precision)
    print("Recall:",list_of_recall)
    print("F1:",list_of_f1)
...
def plot_roc_curve_simple(fprs, tprs):
    ...
    plt.savefig('AE_scenario_s1_d1d1_99.png')
    plt.show()
